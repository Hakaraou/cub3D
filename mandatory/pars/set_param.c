...
/*   Updated: 2024/09/15 19:55:35 by hakaraou         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	set_param(t_cub *cub, char *line, int i)
{
	...
	if (i < 7)
	{
		while (line[j] && !is_white_space(line[j]))
			j++;
		if (!ft_strncmp(line, "NO", j) || !ft_strncmp(line, "SO", j) || !ft_strncmp(line, "WE", j) || !ft_strncmp(line, "EA", j))
		{
			if (set_texture(&cub->texture[i - 1], line) == -1)
				return (-1);
		}
		else if (!ft_strncmp(line, "F", j) || !ft_strncmp(line, "C", j))
		{
			if (set_floor_ceiling(cub, line) == -1)
				return (-1);
		}
	}
