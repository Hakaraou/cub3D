...
/*   Updated: 2024/09/15 19:52:20 by hakaraou         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	check_texture(t_texture *texture)
{
	...
	while (i < 3)
	{
		if (!(ft_strlen(texture[i].path) >= 4 && !ft_strcmp(texture[i].path + ft_strlen(texture[i].path) - 4, ".png")))
		...
		while (j < 4)
		{
			...
			if (texture[j].path && len < ft_strlen(texture[j].path))
				len = ft_strlen(texture[j].path);
			if (!ft_strncmp(texture[i].path, texture[j].path, len))
				return (ft_putendl_fd("ERROR3", 2), -1);
			j++;
		}
		i++;
	}
